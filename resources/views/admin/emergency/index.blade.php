{{-- resources/views/admin/emergency/index.blade.php --}}
<!doctype html>
<html lang="en">
...
<head>
  ...
  <title>Admin • Emergency Protocols · NurSync</title>
...
<body class="min-h-screen bg-slate-50">
      ...
      <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-red-600 text-white shadow-sm">
              <i data-lucide="alert-triangle" class="h-4 w-4"></i>
            </div>
            <div>
              <h1 class="text-[15px] sm:text-[16px] font-semibold leading-tight">Emergency Protocols</h1>
              <p class="text-[12px] text-slate-500 -mt-0.5">
                Manage high-risk emergency algorithms used across all wards and faculty.
              </p>
            </div>
          </div>

          <div class="flex items-center gap-2">
            <a href="{{ route('admin.emergency_protocols.create') }}"
              class="inline-flex items-center gap-2 rounded-xl bg-red-600 text-white px-3 py-2 text-[13px] font-medium shadow hover:bg-red-700 active:scale-[.99]">
              <i data-lucide="plus" class="h-4 w-4"></i>
              <span>Create</span>
            </a>
            <a href="{{ route('admin.emergency_protocols.archived') }}"
            ...
            <form class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between" onsubmit="return false;">
              ...
              <div class="flex flex-col gap-3 md:flex-row md:items-center w-full md:w-auto flex-wrap">
                ...
                <div class="relative flex-1 min-w-[220px] md:w-80">
                  ...
                  <input id="ep-filter-q" type="text" value="{{ $filters['q'] ?? '' }}"
              ...
              <div class="flex items-center gap-3 justify-end">
                <button id="ep-btn-filter" type="button"
  ...
  <script>
    ...
    const EP_INDEX_URL = '{{ route('admin.emergency_protocols.index') }}';
    ...
    async function epFetchList(params = {}) {
      ...
      const stat = params.status ?? (epqs('#ep-filter-status')?.value || '');
      ...
      try {
        ...
        epBindPager();
        epBindDelete();
        epBindArchive();   // bind archive buttons after AJAX load
    ...
    function epBindFilters() {
      ...
      epqs('#ep-filter-status')?.addEventListener('change', () => epFetchList({ page: 1 }));
    ...
    function epBindArchive() {
      epqsa('.ep-archive-form').forEach(form => {
        if (form.dataset.bound === '1') return;
        form.dataset.bound = '1';
        ...
        form.addEventListener('submit', function (e) {
          ...
          if (window.Swal) {
            Swal.fire({
              ...
              reverseButtons: true,
              focusCancel: true,
              confirmButtonColor: '#0f766e',  // teal-ish
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      epBindPager();
      epBindDelete();
      epBindArchive(); // initial bind for archive buttons
