    ...
    <div class="container mx-auto px-8 py-12 space-y-6">
      ...
      <header class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-[24px] sm:text-[26px] font-extrabold tracking-tight text-slate-900">Procedures Library (CI)</h1>
      ...
      @php
        ...
        $list = $procedures->reject(fn($p) => ($p->status ?? 'draft') === 'archived');
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center w-full flex-1">
            ...
            <div class="relative flex-1 sm:w-64 lg:w-80">
              ...
              <input id="searchBox" type="text" placeholder="Search procedures by name, tags, hazards, or area…"
                     class="w-full rounded-xl border-slate-200 pl-9 pr-3 py-2.5 text-sm focus:ring-2 focus:ring-slate-300" />
            </div>
            {{-- Ward --}}
            <select id="wardSelect" class="rounded-xl border-slate-200 text-sm py-2.5 px-3 focus:ring-2 focus:ring-slate-300">
      ...
      @if($list->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center">
          <p class="text-sm text-slate-500">No procedures found. Click <span class="font-semibold">New Procedure</span> to create one.</p>
        </div>
      @else
...
<article
  ...
  data-keywords="{{ Str::of(($p->title.' '.($p->description ?? '').' '.implode(' ', $tags).' '.($p->hazards_text ?? '').' '.$ward))->lower() }}"
>
  <header class="flex items-start justify-between gap-4">
    <div>
      ...
      <div class="mt-2.5 flex flex-wrap items-center gap-2.5">
        @if ($ward)
          <span class="inline-flex items-center rounded-full {{ $wardChip($ward) }} px-2.5 py-0.5 text-[12px]">
            <i data-lucide="hospital" class="h-3.5 w-3.5 mr-1"></i>
            {{ $ward }}
          </span>
        @endif

        <span class="inline-flex items-center rounded-full {{ $statusClasses }} px-2.5 py-0.5 text-[12px] font-semibold">
          {{ $statusLabel }}
        </span>
      </div>
    </div>
  ...
  <div class="mt-auto pt-4 border-t border-slate-100 flex flex-wrap items-center gap-2.5">
    ...
    <a href="{{ route('faculty.procedures.edit', $p->slug) }}"
      ...
      <div id="pagerShell" class="mt-4 flex items-center justify-between text-[12px] text-slate-500">
        <div id="pagerSummary">Showing 0–0 of 0 procedures</div>
        <div class="flex items-center gap-1">
          <button id="btnPrev" class="rounded-lg border px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-transparent" disabled>‹ Prev</button>
          <div id="pageButtons" class="flex items-center gap-1"></div>
          <button id="btnNext" class="rounded-lg border px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-transparent" disabled>Next ›</button>
        </div>
      </div>
...
<script>
  ...
  function buildPageButtons(totalPages) {
    ...
    if (start > 1) {
      ...
      if (start > 2) pageButtons.appendChild(Object.assign(document.createElement('span'), {textContent: '…', className:'px-1 text-slate-400 select-none'}));
    }
    for (let p = start; p <= end; p++) pageButtons.appendChild(makeBtn(String(p), p, currentPage === p));
    if (end < totalPages) {
      if (end < totalPages - 1) pageButtons.appendChild(Object.assign(document.createElement('span'), {textContent: '…', className:'px-1 text-slate-400 select-none'}));
