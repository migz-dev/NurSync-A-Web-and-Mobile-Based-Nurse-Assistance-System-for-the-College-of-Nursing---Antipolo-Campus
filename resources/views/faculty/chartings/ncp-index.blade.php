<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Nursing Care Plan · NurSync (CI)</title>
  @vite(['resources/css/app.css','resources/js/app.js'])
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;600;700&display=swap" rel="stylesheet">
  <style>
    body {
      font-family:'Poppins', ui-sans-serif, system-ui;
    }

    @keyframes slide-in-up {
      from {
        transform: translateY(10px);
        opacity: 0;
      }

      to {
        transform: translateY(0);
        opacity: 1;
      }
    }

    .animate-card-in {
      animation: slide-in-up .35s ease-out both;
      will-change: transform, opacity;
    }
  </style>
</head>
<body class="min-h-screen bg-slate-50">
<main class="min-h-screen flex">
  @include('partials.faculty-sidebar', ['active' => 'chartings'])

  <section class="flex-1">
    <div class="container mx-auto px-8 py-10 space-y-6">

      {{-- HEADER (title + description on the LEFT, Back button on the RIGHT) --}}
      <div class="flex items-center justify-between gap-3">

        {{-- LEFT: Title + description --}}
        <div>
          <h1 class="text-[28px] font-extrabold tracking-tight text-slate-900">
            Nursing Care Plan
          </h1>
          <p class="mt-1 text-xs text-slate-500 max-w-md">
            View ongoing and completed nursing care plans, including diagnoses, goals, interventions, and evaluations.
          </p>
        </div>

        {{-- RIGHT: Back button --}}
        <a href="{{ route('faculty.chartings.patient', $patient) }}"
           class="inline-flex items-center gap-2 rounded-lg border border-slate-300 text-slate-700 px-4 py-2 text-sm hover:bg-slate-50 transition">
          <i data-lucide="arrow-left" class="h-4 w-4"></i>
          <span>Back to Patient</span>
        </a>
      </div>

      {{-- Patient banner --}}
      @include('faculty.chartings._patient-banner')

      @if($records->isEmpty())
        @include('faculty.chartings._empty-state')
      @else
        <div class="space-y-3">
          @foreach($records as $row)
            @php
              $status = ucfirst($row->status ?? 'ongoing');

              $statusBase = 'inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold';
              $statusClass = $statusBase . ' border-slate-200 bg-slate-50 text-slate-600';

              if ($row->status === 'ongoing') {
                  $statusClass = $statusBase . ' border-amber-200 bg-amber-50 text-amber-800';
              } elseif ($row->status === 'resolved') {
                  $statusClass = $statusBase . ' border-emerald-200 bg-emerald-50 text-emerald-800';
              } elseif ($row->status === 'discontinued') {
                  $statusClass = $statusBase . ' border-slate-200 bg-slate-100 text-slate-700';
              }
            @endphp

            <details
              class="group rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow animate-card-in">

              {{-- SUMMARY --}}
              <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-purple-50">
                    <i data-lucide="clipboard-list" class="h-4 w-4 text-purple-600"></i>
                  </span>

                  <div>
                    {{-- Primary Dx + chips --}}
                    <div class="flex flex-wrap items-center gap-2">
                      <span class="text-sm font-semibold text-slate-900">
                        {{ $row->dx_primary }}
                      </span>

                      {{-- Status chip --}}
                      <span class="{{ $statusClass }}">
                        <span class="h-1.5 w-1.5 rounded-full mr-1.5
                          @if($row->status === 'resolved') bg-emerald-500
                          @elseif($row->status === 'ongoing') bg-amber-500
                          @else bg-slate-500
                          @endif
                        "></span>
                        {{ $status }}
                      </span>

                      {{-- Started date --}}
                      @if($row->started_at)
                        <span class="text-[11px] font-medium text-slate-500">
                          Started {{ \Carbon\Carbon::parse($row->started_at)->format('M d, Y') }}
                        </span>
                      @endif
                    </div>

                    {{-- r/t and AEB line --}}
                    @if($row->dx_related_to || $row->dx_as_evidenced_by)
                      <div class="mt-1 text-xs text-slate-500 line-clamp-1">
                        {{ $row->dx_related_to ? 'r/t '.$row->dx_related_to : '' }}
                        {{ $row->dx_as_evidenced_by ? ' AEB '.$row->dx_as_evidenced_by : '' }}
                      </div>
                    @endif
                  </div>
                </div>

                <i data-lucide="chevron-down"
                   class="h-5 w-5 text-slate-400 group-open:rotate-180 transition-transform duration-200"></i>
              </summary>

              {{-- DETAILS --}}
              <div class="px-5 pb-5 pt-3 border-t border-slate-100 text-sm text-slate-700 space-y-3 bg-slate-50/60">

                {{-- Goals / interventions / evaluation --}}
                <div class="grid md:grid-cols-3 gap-3">
                  <div class="rounded-lg bg-white px-3 py-2.5 border border-slate-200">
                    <span class="block text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 mb-1">
                      Goals / Outcomes
                    </span>
                    <p class="text-sm text-slate-800 whitespace-pre-line">
                      {{ $row->goals ?? '—' }}
                    </p>
                  </div>

                  <div class="rounded-lg bg-white px-3 py-2.5 border border-slate-200 md:col-span-1">
                    <span class="block text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 mb-1">
                      Interventions
                    </span>
                    <p class="text-sm text-slate-800 whitespace-pre-line">
                      {{ $row->interventions ?? '—' }}
                    </p>
                  </div>

                  <div class="rounded-lg bg-white px-3 py-2.5 border border-slate-200 md:col-span-1">
                    <span class="block text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 mb-1">
                      Evaluation
                    </span>
                    <p class="text-sm text-slate-800 whitespace-pre-line">
                      {{ $row->outcomes_evaluation ?? '—' }}
                    </p>
                  </div>
                </div>

                {{-- Footer meta --}}
                <div class="flex flex-wrap items-center justify-between gap-2 pt-1">
                  <p class="text-xs text-slate-500">
                    Plan started
                    @if($row->started_at)
                      <span class="font-medium text-slate-700">
                        {{ \Carbon\Carbon::parse($row->started_at)->format('M d, Y') }}
                      </span>
                    @else
                      <span class="font-medium text-slate-700">—</span>
                    @endif
                  </p>

                  <p class="text-xs text-slate-400">
                    Status:
                    <span class="font-semibold text-slate-600">{{ $status }}</span>

                    @if($row->reviewed_at)
                      <span class="mx-1 text-slate-300">•</span>
                      Last reviewed
                      <span class="font-medium text-slate-600">
                        {{ \Carbon\Carbon::parse($row->reviewed_at)->format('M d, Y') }}
                      </span>
                    @endif
                  </p>
                </div>
              </div>
            </details>
          @endforeach
        </div>
      @endif
    </div>
  </section>
</main>

<script src="https://unpkg.com/lucide@latest"></script>
<script>
  try { lucide.createIcons(); } catch (e) {}
</script>
</body>
</html>
