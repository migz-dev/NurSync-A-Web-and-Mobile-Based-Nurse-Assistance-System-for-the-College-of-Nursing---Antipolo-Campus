{{-- resources/views/faculty/assessment_guides/index.blade.php --}}
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
  <title>Assessment Guides · NurSync (CI)</title>
  ...
  <style>
    ...
    /* Smooth card entrance after skeleton hides (same as Ward Orientation / Procedures Library) */
    ...
    <div class="container mx-auto px-8 py-12 space-y-6">
      ...
      <header class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-[24px] sm:text-[26px] font-extrabold tracking-tight text-slate-900">
              Assessment Guides (CI)
            </h1>
            <p class="text-[13px] text-slate-500 mt-1">
              Show student nurses how real nurses are evaluated, how documentation is done, and what safe practice looks like.
            </p>
          </div>
        </div>
        ...
        <div class="hidden sm:flex">
          <a href="{{ route('faculty.instructor.assessment.create') }}"
            ...
            <span>New Assessment Guide</span>
          </a>
        </div>
      </header>
      ...
      @php
        /** @var \Illuminate\Support\Collection|\App\Models\AssessmentGuide[] $guides */
        $guides = $guides ?? collect();

        // 🚫 Do not show archived here (CI manages “active” & “draft” guides)
        $list = $guides->reject(fn($g) => ($g->status ?? 'draft') === 'archived');

        // Status options (for filter dropdown)
        $statusOptions = collect(['all', 'draft', 'published']);

        $filters = $filters ?? ['q' => '', 'status' => 'all'];
        $currentStatus = $filters['status'] ?? 'all';
        $currentSearch = $filters['q'] ?? '';

        // Tag chips palette (used for content themes)
        $tagPalette = [
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center w-full flex-1">
            ...
            <div class="relative flex-1 sm:w-64 lg:w-80">
              ...
              <input id="searchBox" type="text"
                     placeholder="Search guides by title, rubric, documentation tips…"
                     value="{{ $currentSearch }}"
      ...
      @if($list->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center">
          <p class="text-sm text-slate-500">
            No assessment guides yet. Click <span class="font-semibold">New Assessment Guide</span> to create your first CI evaluation guide.
          </p>
        </div>
      @else
      ...
      <div id="cardsGrid" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3 hidden">
        @foreach($list as $g)
          @php
            ...
            // Content themes based on which sections are filled
            $themes = [];
            if (!empty($g->content_rubric))         $themes[] = 'Evaluation rubric';
            if (!empty($g->content_documentation))  $themes[] = 'DAR / SOAP / PIE';
            if (!empty($g->content_tips))           $themes[] = 'Tips from nurses';
...
>
  ...
  <header class="flex items-start justify-between gap-4">
    <div class="flex-1">
      {{-- Fixed-height block so chip row lines up across cards --}}
      <div class="min-h-[90px] flex flex-col gap-1.5">
        ...
        @else
          <p class="text-[13px] text-slate-600 line-clamp-3">
            Guide on how nurses are really evaluated in practice, including documentation and safety expectations.
          </p>
        @endif

        {{-- Status + tag chips row (aligned across cards) --}}
        <div class="mt-1.5 flex flex-wrap items-center gap-2.5 text-[12px]">
          ...
          @if(!empty($g->tags))
            @foreach($g->tags as $t)
              <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] {{ $tagClass($t) }}">
                {{ $t }}
              </span>
            @endforeach
          @endif
  ...
  {{-- Theme chips (optional row, same zone across cards) --}}
  @if(!empty($themes))
  ...
  <div class="mt-auto pt-5 border-t border-slate-100 flex flex-wrap items-center gap-2.5">
    {{-- Open --}}
    <a href="{{ route('faculty.instructor.assessment.edit', $g) }}"
    ...
    <a href="{{ route('faculty.instructor.assessment.edit', $g) }}"
      ...
      <div id="pagerShell" class="mt-4 flex items-center justify-between text-[12px] text-slate-500">
        <div id="pagerSummary">Showing 0–0 of 0 guides</div>
...
<script>
  ...
  function renderPage() {
    ...
    pagerSummary.textContent = `Showing ${humanStart}–${humanEnd} of ${total} guides`;
