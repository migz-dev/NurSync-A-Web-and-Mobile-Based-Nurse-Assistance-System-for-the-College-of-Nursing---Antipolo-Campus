...
<head>
  ...
  <title>Edit Ward Orientation · NurSync (CI)</title>
  ...
  <main class="min-h-screen flex">
    ...
    <section class="flex-1">
      <div class="container mx-auto px-8 py-12 space-y-8 animate-shell">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-[28px] font-extrabold tracking-tight text-slate-900">
                Edit Ward Orientation
              </h1>
              <p class="text-[13px] text-slate-500 mt-1">
                Update how you explain this ward from a nurse’s perspective.
              </p>
            </div>
          </div>
        ...
        {{-- Status badge --}}
        <div class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[12px] text-slate-700">
          <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-slate-900 text-white text-[10px]">
            {{ strtoupper(substr($orientation->status,0,1)) }}
          </span>
          <span>
            Current status: <span class="font-semibold capitalize">{{ $orientation->status }}</span>
          </span>
        </div>

        {{-- Form --}}
        <form action="{{ route('faculty.instructor.orientation.update', $orientation) }}" method="POST" class="space-y-8">
          @csrf
          @method('PUT')
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <input name="title" value="{{ old('title', $orientation->title) }}" required
              ...
              <div>
                ...
                <select name="ward_code"
                  ...
                  @foreach($opts as $code => $label)
                    <option value="{{ $code }}" @selected(old('ward_code', $orientation->ward_code) === $code)>
                      {{ $label }} ({{ $code }})
                    </option>
                  @endforeach
                </select>
            ...
            <div>
              ...
              <textarea name="summary" rows="3"
                        ...
                        placeholder="Brief overview of what this ward orientation will cover...">{{ old('summary', $orientation->summary) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <input type="number" name="estimated_watch_minutes" min="1" max="240"
                       value="{{ old('estimated_watch_minutes', $orientation->estimated_watch_minutes) }}"
                       class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-2.5 text-sm" />
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <textarea name="culture_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('culture_text', $orientation->culture_text) }}</textarea>
              ...
              <div>
                ...
                <textarea name="routines_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('routines_text', $orientation->routines_text) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <textarea name="patient_cases_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('patient_cases_text', $orientation->patient_cases_text) }}</textarea>
              ...
              <div>
                ...
                <textarea name="workload_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('workload_text', $orientation->workload_text) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <textarea name="emergencies_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('emergencies_text', $orientation->emergencies_text) }}</textarea>
              ...
              <div>
                ...
                <textarea name="layout_locations_text" rows="4"
                          class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('layout_locations_text', $orientation->layout_locations_text) }}</textarea>
            ...
            <div>
              ...
              <textarea name="tips_text" rows="3"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm focus:ring-2 focus:ring-slate-200">{{ old('tips_text', $orientation->tips_text) }}</textarea>
