{{-- resources/views/faculty/competencies/index.blade.php --}}
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
  <title>Competency Requirements · NurSync (CI)</title>
  ...
  <style>
    ...
    /* Smooth card entrance after skeleton hides (same as Ward Orientation / Procedures / Assessment Guides) */
  ...
  <section class="flex-1 min-w-0">
    <div class="container mx-auto px-8 py-12 space-y-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-[24px] sm:text-[26px] font-extrabold tracking-tight text-slate-900">
              Competency Requirements (CI)
            </h1>
            <p class="text-[13px] text-slate-500 mt-1">
              Encode the real-life competencies nurses must have before graduation — skills, case logs, and why each one matters.
            </p>
          </div>
        </div>

        {{-- New Competency (desktop) --}}
        <div class="hidden sm:flex">
          <a href="{{ route('faculty.instructor.competencies.create') }}"
      ...
      @php
        ...
        $rotations = $rotations ?? collect();
        $filters = $filters ?? ['q' => '', 'status' => 'all', 'category_id' => 'all', 'rotation' => 'all'];
        ...
        $currentRotation = $filters['rotation'] ?? 'all';
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center w-full flex-1">
            ...
            <div class="relative flex-1 sm:w-64 lg:w-80">
              ...
              <input id="searchBox" type="text"
                     placeholder="Search competencies by skill, category, or reason…"
                     value="{{ $currentSearch }}"
      ...
      @if($items->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center">
          <p class="text-sm text-slate-500">
            No competency requirements encoded yet.
            Click <span class="font-semibold">New Competency</span> to define what nurses must master before graduation.
          </p>
        </div>
      @else
      ...
      <div id="cardsGrid" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3 hidden">
        @foreach($items as $c)
          @php
            ...
            // Theme tags: simple heuristics
            $themes = [];
            if (!empty($c->reason))       $themes[] = 'Why this matters';
            if (!empty($c->description))  $themes[] = 'Skill description';
            if (str_contains(mb_strtolower($cat), 'core'))      $themes[] = 'Core skill';
            if (str_contains(mb_strtolower($cat), 'case'))      $themes[] = 'Case log';
            if (str_contains(mb_strtolower($cat), 'specialty')) $themes[] = 'Specialty';
...
>
    ...
    <div class="flex-1">
      ...
      <div class="min-h-[90px] flex flex-col gap-1.5">
        ...
        <div class="mt-1.5 flex flex-wrap items-center gap-2.5 text-[12px]">
          ...
          {{-- Category --}}
          <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] bg-slate-50 text-slate-700 border-slate-200">
  ...
  <div class="mt-auto pt-5 border-t border-slate-100 flex flex-wrap items-center gap-2.5">
    {{-- Open (edit-as-open) --}}
    <a href="{{ route('faculty.instructor.competencies.edit', $c->id) }}"
