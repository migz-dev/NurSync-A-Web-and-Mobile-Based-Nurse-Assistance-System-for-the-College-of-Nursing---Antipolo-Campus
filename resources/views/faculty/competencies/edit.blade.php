{{-- resources/views/faculty/competencies/edit.blade.php --}}
<!doctype html>
<html lang="en">
...
<head>
  ...
  <title>Edit Competency · NurSync (CI)</title>
...
<main class="min-h-screen flex">
  ...
  <section class="flex-1 min-w-0">
    <div class="container mx-auto px-8 py-12 space-y-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-[22px] sm:text-[24px] font-extrabold tracking-tight text-slate-900">
              Edit Competency Requirement
            </h1>
            <p class="text-[13px] text-slate-500 mt-1">
              Update how this competency is described to student nurses, including why it matters in real practice.
            </p>
          </div>
        </div>
      ...
      @php
        /** @var \App\Models\CompetencyItem $item */
        /** @var \Illuminate\Support\Collection|\App\Models\CompetencyCategory[] $categories */
        $categories = $categories ?? collect();

        // If validation failed, prefer old() explanations over DB
        $oldExpsRaw = old('explanations');
        if (is_array($oldExpsRaw)) {
            $explanations = collect($oldExpsRaw)->map(function ($exp) {
                return (object) [
                    'id'      => $exp['id']      ?? null,
                    'title'   => $exp['title']   ?? '',
                    'content' => $exp['content'] ?? '',
            ...
            $explanations = $item->explanations ?? collect();
        }
      ...
      <div class="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 md:p-7 lg:p-8 opacity-0 animate-card-in">
        <form method="POST"
              action="{{ route('faculty.instructor.competencies.update', $item) }}"
              class="space-y-6">
          @csrf
          @method('PUT')
          ...
          <div class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                @php
                  $oldStatus = old('status', $item->status ?? 'draft');
                @endphp
                <select
                  id="status"
                  name="status"
                ...
                <p class="mt-1 text-[11px] text-slate-500">
                  Drafts are visible only to you. Published items can be surfaced to student view (read-only).
                </p>
              </div>
            </div>
          </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <div class="md:col-span-1">
                ...
                <textarea
                  ...
                  class="mt-1 block w-full rounded-xl border-slate-200 text-sm py-2.5 px-3 focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 resize-none">{{ old('description', $item->description) }}</textarea>
              </div>
              ...
              <div class="md:col-span-1">
                ...
                <textarea
                  ...
                  class="mt-1 block w-full rounded-xl border-slate-200 text-sm py-2.5 px-3 focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 resize-none">{{ old('reason', $item->reason) }}</textarea>
              </div>
            </div>

            @if(!empty($rotations))
              <p class="text-[11px] text-slate-500">
                Tip: You can connect this competency to rotation-specific skills and RLE case requirements later
          ...
          <div class="space-y-4">
            ...
            <div id="explanationsContainer" class="space-y-3">
              @foreach($explanations as $idx => $exp)
                @php
                  $expId = $exp->id ?? null;
                  $expTitle = old("explanations.$idx.title", $exp->title ?? '');
                  $expContent = old("explanations.$idx.content", $exp->content ?? '');
                @endphp
                <div
                  class="explanation-card rounded-xl border border-slate-200 bg-slate-50/60 p-4 md:p-5 animate-card-in"
                  data-existing="{{ $expId ? '1' : '0' }}">
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 space-y-3">
                      @if($expId)
                        <input type="hidden"
                               name="explanations[{{ $idx }}][id]"
                               value="{{ $expId }}">
                      @endif
                      ...
                      <div>
                        ...
                        <textarea
                          name="explanations[{{ $idx }}][content]"
                          rows="4"
                          placeholder="Write this as if you’re explaining to a student nurse at the bedside."
                          class="mt-1 block w-full rounded-xl border-slate-200 text-sm py-2 px-3 focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 resize-none">{{ $expContent }}</textarea>
                      </div>
                    </div>
...
<script>
  ...
  // Remove / mark-for-delete handler
  explanationsContainer?.addEventListener('click', (e) => {
    ...
    if (isExisting && idInput && idInput.value) {
      // Mark existing explanation for deletion: clear fields but keep hidden id
      ...
      note.className = 'mt-2 text-[11px] text-rose-600';
      note.textContent = 'This explanation will be removed when you save.';
      card.appendChild(note);

      btn.remove(); // remove delete button
    } else {
      // New unsaved explanation – just remove from DOM
