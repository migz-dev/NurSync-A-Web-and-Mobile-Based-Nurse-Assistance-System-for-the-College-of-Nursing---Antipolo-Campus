{{-- resources/views/faculty/emergency/edit.blade.php --}}
<!doctype html>
<html lang="en">
...
<head>
  ...
  <title>Edit Emergency Protocol · NurSync</title>
    ...
    <div class="container mx-auto px-8 py-12 space-y-6">
      ...
      <header class="space-y-4">
        <div class="flex items-center justify-between gap-3">
          <button type="button"
                  onclick="window.location.href='{{ route('faculty.emergency.show', $protocol->slug) }}'"
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="mt-1 text-[13px] text-slate-600 max-w-3xl">
              Update the title, category, ward, severity, and step-by-step actions for this protocol.
            </p>
      ...
      @php
        ...
        // Wards list (fallback if controller didn't pass it)
        $wards = $wards ?? [
      ...
      <form id="ep-edit-form"
            method="POST"
            action="{{ route('faculty.emergency.update', $protocol->slug) }}"
              ...
              <div>
                ...
                <select id="ep-ward" name="ward"
                  ...
                  @foreach ($wards as $ward)
                    <option value="{{ $ward }}" @selected(old('ward', $protocol->ward) === $ward)>
                      {{ $ward }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select id="ep-severity" name="severity"
                          class="w-full rounded-xl border-slate-200 px-3 py-2.5 text-sm focus:ring-2 focus:ring-slate-300"
                          required>
                    @foreach ($severities as $sevOption)
                      <option value="{{ $sevOption }}" @selected(old('severity', $protocol->severity) === $sevOption)>
                        {{ $sevOption }}
                      </option>
                    @endforeach
                  </select>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          ...
          <p class="text-[11px] text-slate-500 mb-3">
            Keep each step clear and action-oriented. Order follows the list.
          </p>
