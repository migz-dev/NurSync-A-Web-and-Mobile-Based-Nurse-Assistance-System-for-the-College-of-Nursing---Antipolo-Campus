...
<head>
  ...
  <title>My Clinical Experience · NurSync (CI)</title>
  ...
  <section class="flex-1 min-w-0">
    <div class="container mx-auto px-8 py-12 space-y-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[13px] text-slate-500 mt-1">
              Turn your real clinical stories into guided lessons that student nurses can revisit anytime.
            </p>
          </div>
        </div>

        <a href="{{ route('faculty.instructor.experiences.create') }}"
           class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 text-white text-[13px] font-semibold px-4 py-2.5 shadow-sm hover:bg-emerald-700">
          <i data-lucide="plus" class="h-4 w-4"></i>
          <span>New Clinical Story</span>
        </a>
      </header>
      ...
      @if($list->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center">
          <p class="text-sm text-slate-500">
            You haven’t shared any clinical experiences yet. Start by creating your first story so students can learn from your real practice.
          </p>
          <div class="mt-4">
            <a href="{{ route('faculty.instructor.experiences.create') }}"
               class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 text-white text-[13px] font-semibold px-4 py-2.5 shadow-sm hover:bg-emerald-700">
        ...
        @foreach($list as $exp)
          ...
          >
                ...
                <div class="min-h-[90px] flex flex-col gap-1.5">
                  ...
                  <p class="text-[13px] text-slate-600 line-clamp-3">
                    @if(!empty($exp->summary))
                      {{ $exp->summary }}
                    @else
                      Reflection on a real clinical situation, captured to guide future student nurses.
                    @endif
                  </p>
            ...
            <div class="mt-auto pt-5 border-t border-slate-100 flex items-center gap-2">
              <a href="{{ route('faculty.instructor.experiences.edit', $exp) }}"
              ...
              <a href="{{ route('faculty.instructor.experiences.show', $exp) }}"
                 class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3.5 py-2 text-[13px] text-slate-700 hover:bg-slate-50">
      ...
      <div id="pagerShell" class="mt-4 flex items-center justify-between text-[12px] text-slate-500">
        <div id="pagerSummary">Showing 0–0 of 0 experiences</div>
...
<script>
  ...
  function renderPage() {
    ...
    pagerSummary.textContent = `Showing ${humanStart}–${humanEnd} of ${total} experiences`;
