{{-- resources/views/student/return-demo/show.blade.php --}}
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
  <title>{{ $skill->title }} · Return Demo · NurSync (Student)</title>
  ...
  <style>
    ...
    /* Same animation style used on CI / Return Demo index */
...
<main class="min-h-screen flex">
  ...
  <section class="flex-1 min-w-0">
    <div class="container mx-auto px-8 py-12 space-y-6" data-anim-page>
      @php
        $ward   = $skill->clinical_wards ?: 'Unassigned';
        ...
        // PPE flatten
        $ppeRaw  = (array) ($skill->ppe_json ?? []);
        $ppeFlat = collect($ppeRaw)->flatMap(fn($i) => is_array($i) ? $i : [$i])
                                   ->filter()
                                   ->unique()
                                   ->values()
                                   ->all();

        // Robust tags decode (same as index cards)
        $tagsRaw = $skill->tags_json ?? [];
        if (is_array($tagsRaw)) {
            $tags = $tagsRaw;
        ...
        $hasVideo = !empty($skill->video_path) || !empty($skill->video_url);
        $hasPdf   = !empty($skill->pdf_path);
      @endphp

      {{-- Back + heading (mirror CI show) --}}
      <header class="space-y-4">
        <div class="flex items-center justify-between gap-3">
          <button type="button"
                  onclick="window.location.href='{{ route('student.return_demo.index') }}'"
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <div class="mt-3 flex flex-wrap items-center gap-2">
                <span class="inline-flex items-center rounded-full {{ $wardChip($skill->clinical_wards) }} px-2 py-0.5 text-[11px]">
                ...
                <span class="inline-flex items-center rounded-full bg-slate-50 px-2 py-0.5 text-[11px] text-slate-700">
                  ...
                  Updated {{ $skill->updated_at?->diffForHumans() ?? '—' }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="text-right text-[11px] text-slate-500">
            <div>Assigned: {{ $skill->created_at?->format('M d, Y H:i') ?? '—' }}</div>
          </div>
        </div>
      </header>

      {{-- Tags + meta links (CI-style but student-safe) --}}
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3 text-[11px] text-slate-500">
          @if($hasVideo)
            <span class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-2.5 py-1">
              <i data-lucide="video" class="h-3 w-3"></i>
              Demo available
            </span>
          @endif
          @if($hasPdf)
            <a href="{{ asset($skill->pdf_path) }}" target="_blank"
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            ...
            @php $haz = trim((string)($skill->hazards_text ?? '')); @endphp
            @if (!empty($skill->description) || $haz)
              @if (!empty($skill->description))
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
            ...
            <dl class="grid grid-cols-1 gap-2 text-[12px] text-slate-600">
              ...
              <div class="flex justify-between">
                ...
                <dd class="font-medium">{{ $skill->steps?->count() ?? 0 }}</dd>
              </div>
            </dl>

            <p class="mt-2 text-[11px] text-slate-500">
              Use this guide to prepare for your scheduled Return Demonstration with your Clinical Instructor.
            </p>
          </div>
        </div>

        {{-- Right column: Steps + Media --}}
        <div class="space-y-4">
          {{-- Steps: accordion + per-step video --}}
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div class="flex items-center justify-between mb-3">
              ...
              <span class="text-[11px] text-slate-500">
                Tap a step to expand details & video.
              </span>
            </div>

            @if(!$skill->steps || $skill->steps->isEmpty())
              <p class="text-[13px] text-slate-500">No steps added yet.</p>
            @else
              <ol class="space-y-2" id="stepAccordion">
                @foreach($skill->steps->sortBy('step_no') as $step)
                  @php
                    $panelId   = 'step-panel-'.$step->id;
                    $hasStepVideo = !empty($step->video_url) || !empty($step->video_path);
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <h2 class="text-[14px] font-semibold text-slate-900 mb-2 flex items-center gap-2">
              <i data-lucide="video" class="h-4 w-4 text-slate-500"></i>
              Demo Video
            </h2>
            @if(!empty($skill->video_path))
              <video
                class="w-full rounded-xl border border-slate-200 bg-black/5"
                style="aspect-ratio: 16 / 9;"
                controls playsinline preload="metadata">
                <source src="{{ asset($skill->video_path) }}">
                Your browser does not support the video tag.
              </video>
            @elseif(!empty($skill->video_url))
            ...
            @else
              <div class="aspect-video rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 text-[12px]">
                No video attached for this skill.
              </div>
            @endif
          </div>

          {{-- Attachments --}}
          @if($skill->attachments && $skill->attachments->isNotEmpty())
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
              <h2 class="text-[14px] font-semibold text-slate-900 mb-2 flex items-center gap-2">
                <i data-lucide="paperclip" class="h-4 w-4 text-slate-500"></i>
                Attachments
              </h2>
              <ul class="mt-2 text-[13px] text-slate-700 space-y-2">
                @foreach($skill->attachments as $a)
                  <li>
                    <a href="{{ asset($a->path) }}" target="_blank"
                       class="inline-flex items-center gap-2 text-slate-700 hover:text-slate-900 underline">
                      <i data-lucide="file" class="h-4 w-4"></i>
                      {{ $a->label ?: basename($a->path) }}
                    </a>
                  </li>
                @endforeach
              </ul>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <a href="{{ route('student.return_demo.index') }}"
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Accordion behavior – one step open at a time
    ...
    toggles.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.js-step-chevron').forEach(icon => icon.classList.remove('rotate-180'));
